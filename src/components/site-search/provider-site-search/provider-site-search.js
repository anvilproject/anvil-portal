...
function ProviderSiteSearch(props) {
    ...
    const onInitializeInputValue = useCallback(() => {
        ...
        if ( query ) {
            ...
            setInputValue(query);
            setSiteSearch(siteSearch => ({...siteSearch, searchError: false, searchLoading: true, searchPage: 1, searchTerms: query}));
        }
    }, [currentLocation]);
    ...
    useEffect(() => {
        ...
        if ( searchLoading && searchTerms ) {
            ...
            const GCSERequestURL = AnVILGCSEService.getGCSERequestURL(searchTerms, searchPage);
            ...
            fetch(GCSERequestURL)
                ...
                .then(res => {
                    ...
                    setSiteSearch(siteSearch => ({...siteSearch, searchError: false, searchLoading: false}));
                })
                .catch(err => {
        ...
        if ( searchLoading && !searchTerms ) {
            ...
            const delayProgressIndicatorFinish = setTimeout(() => {
                ...
                setSiteSearch(siteSearch => ({...siteSearch, searchError: true, searchLoading: false}));
            }, 1000);
    ...
    return (
        <ContextSiteSearch.Provider value={{inputValue, onSetInputValue, onSetSiteSearchBarOpen, onSiteSearchPageRequest, onSubmitSiteSearch,
            nextPage, previousPage, searchBarOpen, showPagination, siteSearch, siteSearchResults}}>
            {children}
