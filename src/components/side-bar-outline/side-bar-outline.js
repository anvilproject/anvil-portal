...
function SideBarOutline(props) {
    ...
    const outlines = OutlineService.filterHtmlAstByHeading(OutlineStaticQuery(), docPath);
                ...
                <div className={compStyles.outline}>
                    ...
                    <ul className={compStyles.outlineList}>
                        {outlines.map((outlineItem, o) =>
                            <SideBarOutlineItem key={o} activeOutline={activeOutline} outlineItem={outlineItem}/>)}
                    </ul>
                </div>
            </Scroller> : null
    );
