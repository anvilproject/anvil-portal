...
- **File Manifest Download** — Export a manifest containing the S3 URI for each file. A new column, `files.azul_mirror_uri`, provides the S3 URI for each file in the S3 bucket. Note that the filename in this column is a hash; the human-readable filename can be found in the `files.file_name` column.
