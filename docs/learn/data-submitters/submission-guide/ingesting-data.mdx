...
You’ll work with a designated point of contact (POC) at the AnVIL team to shepherd the data (omic data and image files and TSV load files) into the deposit workspace (and ultimately the AnVIL data storage repository). Note that because each engagement will most likely be different, we will be further developing and refining (as needed) processes as we engage with submitters.
