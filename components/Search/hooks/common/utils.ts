...
export function getSearchIndex(
  ...
  const value = searchParams?.get(SEARCH_PARAMETERS.START);
  // Only accept a plain run of digits; Number() would coerce forms like "0x10"
  // or "1e1" into unintended integers.
  if (!value || !/^\d+$/.test(value)) return SEARCH_ENGINE_MIN_START_INDEX;
  const searchIndex = Number(value);
  return Math.min(
    Math.max(searchIndex, SEARCH_ENGINE_MIN_START_INDEX),
    SEARCH_ENGINE_MAX_START_INDEX
  );
}
