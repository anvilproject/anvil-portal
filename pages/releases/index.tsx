...
function buildReleases(
  ...
  return dates.map((date) => {
    const slug = pathsByDate.get(date) ?? [];
    const title = getReleaseTitle(slug, date);
    return {
      id: getReleaseId(date),
      month: format(date, "MMMM"),
      timeSinceRelease: isPast(date)
...
/**
 * Returns the release title from frontmatter.
 * @param segments - Slug segments.
 * @param date - Release date (fallback).
 * @returns release title.
 */
function getReleaseTitle(segments: string[], date: Date): string {
  const fileName = buildMDXFilePath([DOCS_DIR, RELEASES_DIR], segments);
  if (!fileName) return `${format(date, "MMMM")} Release`;
  const { data } = getMatter(fileName);
  return data.title || `${format(date, "MMMM")} Release`;
}
