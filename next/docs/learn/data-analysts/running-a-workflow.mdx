...
<Video
  src="https://www.youtube.com/embed/CT82tcJTA0c"
  title="Week 3: Running a Workflow"
/>

<Alert icon={false} severity="info" title="Notes">
  1. Visit the [course
  schedule](/events/bioconductor-popup-workshops-20210510#other-sessions) for
  links to the recorded session, and to other workshops in the series. 1. The
  material below requires a billing account. We provide a billing account during
  the workshop, but if you're following along on your own see '[Next
  Steps](/learn/data-analysts/running-a-workflow#next-steps)' for how to create
  a billing account. 1. Access to the workspaces we use may require
