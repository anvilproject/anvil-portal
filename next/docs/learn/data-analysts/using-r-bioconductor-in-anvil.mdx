...
<Subheader>Martin Morgan</Subheader>
...
<Video
  src="https://www.youtube.com/embed/8Ccj__2GqJ4"
  title="Week 1: Using R / Bioconductor in AnVIL"
/>

**Notes**

1. The material below requires a billing account. We provide a billing account during the workshop, but if you're following along on your own see '[Next Steps](/learn/data-analysts/using-r-bioconductor-in-anvil#next-steps)' for how to create a billing account.
...
- Visit [https://anvil.terra.bio](https://anvil.terra.bio) to use the AnVIL platform.
...
- Visit [https://anvil.terra.bio/](https://anvil.terra.bio/).
- Expand the 'HAMBURGER' menu (blue circle, below).
  ![Expand the Menu](/consortia/learn/data-analysts/using-bioconductor-menu.png)
- Click the 'Sign in with Google' icon and follow the prompts.
  ![Sign In With Google](/consortia/learn/data-analysts/using-bioconductor-sign-in.png)
...
- AnVIL data and computing resources are organized around Workspaces. Once you've signed, in, choose 'Workspaces' under the HAMBURGER menu.
  ![Choose Workspaces](/consortia/learn/data-analysts/using-bioconductor-workspaces-tab.png)
...
- If you registered for the workshop with an email address known to AnVIL / Terra, you'll see the Bioconductor-Workshops-PopUp workspace under 'MY WORKSPACES'.
  ![Bioconductor Workshop](/consortia/learn/data-analysts/using-bioconductor-workspace-workshop.png)
- Enter the workspace by clicking on the [Bioconductor-Workshop-PopUp](https://anvil.terra.bio/#workspaces/deeppilots-bioconductor/Bioconductor-Workshop-PopUp) link. There are many components to the workspace; we'll cover many of these over the course of the PopUp workshops.
  ![Enter the Workspace](/consortia/learn/data-analysts/using-bioconductor-enter-workspace.png)
- Start by making a _clone_ so that we can perform computations on our own copy of the workspace. Do this by clicking on the TEARDROP (three vertical dots) in the top right of the page, and choose 'Clone'.
  ![Start by Making a Clone](/consortia/learn/data-analysts/using-bioconductor-clone-menu.png)
- If you see something like the following, then customize the 'Workspace name' to a globally unique name. For instance, I changed `copy` to `-mtmorgan-popup`. It's convenient NOT to have spaces in a workspace name.
  ![Customize Workspace Name](/consortia/learn/data-analysts/using-bioconductor-customize-workspace-name.png)
  If instead, you see a 'Billing project' that is NOT deeppilots-bioconductor, or if you see something like
  ![Set up Billing Message](/consortia/learn/data-analysts/using-bioconductor-billing-message.png)
  then contact the workshop organizer with your AnVIL email address to be added to the deeppilots-bioconductor billing project. See the [Frequently Asked Questions](/learn/data-analysts/using-r-bioconductor-in-anvil#frequently-asked-questions), below, for more information on billing projects.
- Return, via the HAMBURGER menu or by clicking on the WORKSPACES element at the top of the page, to the list of WORKSPACES available to you. You'll see your own version of the workspace. Open it.
  ![Open the Cloned Workspace](/consortia/learn/data-analysts/using-bioconductor-open-cloned-workspace.png)
...
- Navigate to the NOTEBOOKS tab of your workspace. Click on the RUN Cloud Environment icon.
  ![Run Cloud Environment](/consortia/learn/data-analysts/using-bioconductor-run-cloud-environment.png)
- Create a Cloud Environment with a single CPU, modest memory, and a _persistent disk_ to perform computation with.
  ![Create a Cloud Environment](/consortia/learn/data-analysts/using-bioconductor-create-cloud-environment.png)
- Compute environments can be customized, e.g., to 96 CPUs, 624 GB of memory, and very large disks (!). Of course this costs more…
  ![Customized Cloud Environments](/consortia/learn/data-analysts/using-bioconductor-customized-cloud-environment.png)
- After pressing the CREATE button, note the Cloud Environment icon at the top right of the NOTEBOOKS tab. Initially, it indicates that the cloud environment is being created...
  ![Cloud Environment Icon in Creating Indicator](/consortia/learn/data-analysts/using-bioconductor-cloud-environment-icon-creating-mode.png)
  ...but after 1 or two minutes the necessary cloud resources have been obtained and are ready for your use.
  ![Cloud Environment Icon in Ready Indicator](/consortia/learn/data-analysts/using-bioconductor-cloud-environment-icon-running.png)
...
- With the cloud environment running, click on the Create a New Notebook button, name the notebook and choose R (of course!) as the notebook language.
  ![Create a New Notebook](/consortia/learn/data-analysts/using-bioconductor-new-notebook.png)
- Use the TEARDROP beside the newly created notebook to open it in Edit mode.
  ![Open Notebook in Edit Mode](/consortia/learn/data-analysts/using-bioconductor-open-edit-mode.png)
- The Jupyter notebook interface is pictured below. Note the information icons to the top left, showing that we're using an R kernel and that the notebook is editable. We'll use the toolbar widgets to the right to edit the notebook, entering text into cells in the body of the notebook.
  ![Jupyter Notebook Interface](/consortia/learn/data-analysts/using-bioconductor-jupyter-notebook-interface.png)
...
- Here I've entered a simple mathematical expression in the first cell and pressed the Run tool. This evaluated the expression and opened a new cell. I used the Cell tool to switch to Markdown and entered some text.
  ![Run a Simple Mathematical Expression](/consortia/learn/data-analysts/using-bioconductor-run-simple-expression.png)
- I pressed the Run tool again, entered some R commands, etc, to end up with the following notebook.
  ![Run with R Commands](/consortia/learn/data-analysts/using-bioconductor-run-with-r-commands.png)
...
- Return to the workspace DASHBOARD and click on the Cloud Environment widget.
  ![Cloud Environment Widget](/consortia/learn/data-analysts/using-bioconductor-cloud-environment-widget.png)
- Select the RStudio custom environment and click NEXT.
  ![RStudio Custom Environment](/consortia/learn/data-analysts/using-bioconductor-rstudio-custom-environment.png)
- What's happening?
...
- Launch RStudio.
  ![Launch RStudio](/consortia/learn/data-analysts/using-bioconductor-launch-rstudio.png)
- Our old friend...
  ![RStudio](/consortia/learn/data-analysts/using-bioconductor-rstudio.png)
...
- Is it possible to use a custom docker image? Yes. [DataBiosphere/terra-docker](https://github.com/databiosphere/terra-docker#terra-base-images) contains suitable R / Jupyter base images; [anvilproject/anvil-docker](https://github.com/anvilproject/anvil-docker) contains RStudio images. Select the image as part of a 'Custom' Cloud Environment.
  ![Custom Docker Image](/consortia/learn/data-analysts/using-bioconductor-custom-docker-image.png)
