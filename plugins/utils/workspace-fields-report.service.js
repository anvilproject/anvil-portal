...
const allowListWorkspaceFields = ["library:dataUseRestriction", "library:diseaseOntologyLabel", "library:datatype"];
...
const generateWorkspaceFieldsReportCSV = async function generateWorkspaceFieldsReportCSV() {
    ...
    if ( fs.existsSync(path.resolve(__dirname, workspacesFileName)) ) {
        ...
        const fieldsByWorkspace = workspacesJSON
            ...
            .reduce((acc, workspace) => {
                ...
                const workspaceString = `${name}${buildWorkspaceFieldsString(attributes)}`;
    ...
    /* Return - file doesn't exist and plugin will not proceed. */
        ...
        console.log("Generation of workspace fields report unable to run: workspaces JSON file not found.");
    }
};
